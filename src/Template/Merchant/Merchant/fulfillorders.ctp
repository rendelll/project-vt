...
<div class="row page-titles m-b-20">
  <div class="col-md-6 col-lg-12 align-self-center">
      <h3 class="text-themecolor m-b-0 m-t-0"><?php echo __d('merchant','Fulfil Orders'); ?></h3>
      <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active"><?php echo __d('merchant','Fulfil Orders'); ?></li>
      </ol>
  </div>
</div>
	...
	<div class="col-md-12">
		...
		<div class="card">  
			<div class="card-block">
     			<h4 class="text-themecolor m-b-0 m-t-0 clearfix"> <?php echo __d('merchant','Fulfil Orders'); ?>
     			<?php
	        	if($oldordercount>0)
	        	{
	        	?>
	     			<ul class="nav nav-pills" style="display: inline-flex; float: right;">
	        			<li class=" nav-item"> 
	        				<a href="<?php echo MERCHANT_URL; ?>/fulfilloldorders" class="nav-link"><?php echo __d('merchant','View Old Orders'); ?></a> 
	        			</li>
	    			</ul>
	    		<?php } ?>
	 			</h4>
	 	    	<hr/>
           		...
           		<div class="prvcmntcont m-t-30"> 
         			...
         			echo '<span class="col-sm-3 p-l-0"> '.__d('merchant','Filters').'<!-- input type="text" id="ordersearchval" style="margin-top:7px;"  --> </span>
						...
						<!-- button class="btn btn-success" onclick="fulfillorder_search()" style="vertical-align:inherit;">Search</button>
						<a class="btn btn-info clearResult" href="'.MERCHANT_URL.'/fulfillorders" style="vertical-align:inherit;">Reset</a -->';
					?>
        	...
        	<table id="myTable" class="table table-bordered table-striped">
        		<thead>
        			<tr>
						...
						<!-- th><?php // echo __('Color/Size');?></th -->
						<th><?php echo __d('merchant','Buyer');?></th>
						<th><?php echo __d('merchant','Barcode number');?></th>
				...
				foreach($orderDetails as $ky=>$orderDetail){
					...
					<tr id="odr<?php echo $orderid; ?>">
						...
						<td style="text-align:center;"><?php echo $orderDetail['buyer']; ?></td>
						<td style="text-align:center;"><?php if(!empty($orderItem['ean'])) echo $orderItem['ean']; else echo " - "; ?></td>
						...
						<td class="status<?php echo $orderid; ?>" style="width: 70px;color:<?php echo $statusColor; ?>;"><?php 
							...
							?></td>
						<td class="lastmoreoptiontd" style="width:100px !important;">
							<div class="moreactionmyord">
								<a href="javascript:void(0);" class="moreactions" style="cursor: pointer;" onclick="openmenu('<?php echo $orderid; ?>');">
								...
								<div class="moreactionlistmyord moreactionlistmyord<?php echo $orderid; ?>">
									<ul class="leftcell list-unstyled m-0">
										<?php if ($orderDetail['status'] == '' || $orderDetail['status'] == 'Pending' && $orderDetail['deliverytype'] != 'pickup') {?>
										<li class="moreactionsli curpoint Processing<?php echo $orderid; ?>" onclick="markprocess('<?php echo $orderid; ?>','Processing')"><?php echo __d('merchant','Mark Process');?>
										...
										<?php
										if($orderDetail['deliverytype'] == 'pickup' && $orderDetail['status'] != 'Paid')
										{
										if($orderDetail['paymentmethod'] == 'COD') {
								        ?>
											<li class="moreactionsli curpoint paid<?php echo $orderid; ?>" onclick="markpaid('<?php echo $orderid; ?>')"><?php echo __d('merchant','Mark as Delivered');?>
												<img class="paidinv-loader-<?php echo $orderid; ?>" src="<?php echo SITE_URL; ?>images/loading.gif" style="display: none; float: right; width: 12px; padding-top: 6px;">
											</li>
										<?php } else { ?>
											<li class="moreactionsli curpoint paid<?php echo $orderid; ?>" onclick="markprocess('<?php echo $orderid; ?>','Delivered')"><?php echo __d('merchant','Mark as Delivered');?>
												<img class="paidinv-loader-<?php echo $orderid; ?>" src="<?php echo SITE_URL; ?>images/loading.gif" style="display: none; float: right; width: 12px; padding-top: 6px;">
											</li>
