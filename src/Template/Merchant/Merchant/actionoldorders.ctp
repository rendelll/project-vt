...
<div class="row page-titles m-b-20">
  <div class="col-md-6 col-lg-12 align-self-center">
      <h3 class="text-themecolor m-b-0 m-t-0"><?php echo __d('merchant','New Orders'); ?></h3>
      <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="<?php echo MERCHANT_URL;?>/actionrequired"><?php echo __d('merchant','New Orders'); ?></a></li>
			...
			<div class="card-block">
     			<h4 class="text-themecolor m-b-0 m-t-0 clearfix"> <?php echo __d('merchant','Old Orders'); ?>
	     			<ul class="nav nav-pills" style="display: inline-flex; float: right;">
	        			<li class=" nav-item"> 
	        				<a href="<?php echo MERCHANT_URL; ?>/neworders" class="nav-link"><?php echo __d('merchant','View recent orders'); ?></a> 
	        			</li>
	    			</ul>
	 			</h4>
	 	    	<hr/>

        		<!-- div class="markshiphead m-b-15" style="font-size:12px;">
    				<?php echo __('My Sales');?>
    				<span class="myordertimeline"> (<?php echo __('Sales History');?>)
    				</span>
    			</div -->
         		...
         		<div class="prvcmntcont m-t-30"> 
                    ...
                    <?php if (count($orderDetails) > 0) { ?>
         			<?php  echo '<span class="col-sm-3 p-l-0"> '.__d('merchant','Filters').' <!-- input type="text" id="ordersearchval" style="margin-top:7px;"  --> </span>
						...
						<!-- button class="btn btn-success" onclick="actionoldorder_search()" style="vertical-align:inherit;">Search</button>
						<a class="btn btn-info clearResult" href="'.MERCHANT_URL.'/actionoldorders" style="vertical-align:inherit;">Clear</a -->'; } ?>
        	...
        	<table id="myTable" class="table table-bordered table-striped">
				...
				foreach($orderDetails as $ky=>$orderDetail){
					...
					<tr>
						...
						<td class="status<?php echo $orderid; ?>" style="width: 70px;color:<?php echo $statusColor; ?>;"><?php echo __d('merchant',$orderStatusCurrent);
							?></td>
							...
							<div class="moreactionmyord">
								...
								<div class="moreactionlistmyord moreactionlistmyord<?php echo $orderid; ?>">
									<ul class="leftcell list-unstyled m-0">
										...
										?>
										<li class="moreactionsli curpoint Shipped<?php echo $orderid; ?>" onclick="markprocess('<?php echo $orderid; ?>','Shipped')"><?php echo __d('merchant','Mark Shipped');?></li>
										<?php } 
											if ($orderDetail['status'] != 'Delivered' && $orderDetail['status'] != 'Paid') {
										?>
										<li class="moreactionsli curpoint" onclick="markprocess('<?php echo $orderid; ?>','Track')"><?php echo __d('merchant','Add Tracking');?></li>
										<?php } ?>
