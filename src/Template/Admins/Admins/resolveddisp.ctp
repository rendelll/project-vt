...
<div class="col-lg-12 p-0">
            <div class="card card-outline-info">
                <div class="card-header">
                        <h4 class="m-b-0 text-white"><?php echo __d('admin', 'Resolved Disputes'); ?></h4>
                 </div>
	...
	echo  '<div class="table-responsive m-t-0">';
			...
			echo '<thead>';
				echo "<tr>";
					...
					echo "<th c lass='aligncenter' style='cursor:pointer;'>".__d("admin", "Action")."</th>";
				...
				if(count($order_data)!=0){
					...
					echo "<tr id='scatgys".$disid."'>";
											...
											<?php
											if(empty($uname))
												echo "<td class='aligncenter'>NA</td>";
											else
												echo "<td class='aligncenter'>".$uname."</td>";
											if(empty($sname))
												echo "<td>".__d("admin", "NA")."</td>";
											else
											...
											echo "<td class='aligncenter' style='max-width:150px;word-wrap: break-word;'>".$pbm."</td>";
...
function disputestatuscurrent(disid){
	//alert(disid);

	if (confirm("<?php echo __d('admin', 'Are you sure you want to Change this Dispute?'); ?>")) {
	...
	//alert(status);
	//alert(statuscurrent);
	$(status).html(statuscurrent);


	$.ajax({
	      url: BaseURL+"dispcurrentstatus",
	      type: "post",
	      data : { 'disid': disid, 'statuscurrent': statuscurrent},
