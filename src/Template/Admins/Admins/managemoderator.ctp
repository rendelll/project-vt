...
<div class="col-lg-12 p-0">
            <div class="card card-outline-info">
                <div class="card-header">
                        <h4 class="m-b-0 text-white"><?php echo __d('admin', 'Approved Moderators'); ?></h4>
                 </div>
...
<?php
if (!empty($getmsg)) {
    if($getmsg[0] == 'modaddsuccess')
    {
     $succmsg = "Moderator added successfully";
     echo '<div id="alertmsg" class="successmsg">'.$succmsg.'</div>';
     }
     else if($getmsg[0] == 'appmodeditsuccess')
     {
      $succmsg = "Moderator updated successfully";
     echo '<div id="alertmsg" class="successmsg">'.$succmsg.'</div>';
     }
...
?>
                                     ...
                                     <!--       <div class="col-lg-6">
                                                <div class="input-group">
                                                    <input type="text" class="form-control" id='usersrchSrch' onkeyup='search_moderator()'placeholder="Search for...">
                                                    <span class="input-group-btn">
                          <button class="btn btn-info" type="button" onclick='search_moderator();'><?php echo __d('admin', 'Search'); ?></button>
                        </span>
                                                </div>
                                            </div>
                                        </div>-->
    ...
    if($srcs!="")
    {
        ...
        echo '<br /><br />';
                  ...
                  echo  '<div class="table-responsive m-t-0">';
                    echo '<table id="approvedmoderatorstable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">';
                        echo '<thead>';
                        ?>
      ...
      <?php if(count($userdet)!=0){
            foreach($userdet as $user_det){
                ...
                echo '<tr id="del_'.$id.'" style="'.$style.'">';
                    ...
                    if($demo_active!='yes')
                    echo '<a onclick="deleteusrlists('.$id.',\'approvedmoderator\')"><input type="button" class="btn btn-rounded btn-outline-danger" style="width:auto; font-size: 11px;" value="'.__d('admin', 'Delete').'"></a>';
                    echo '</td>';
                echo '</tr>';
            }
            }else{
...
function myFunction() {
  ...
  <script>
    ...
    $('#approvedmoderatorstable').DataTable({
    "bInfo" : false,
