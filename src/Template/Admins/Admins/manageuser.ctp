...
<div class="col-lg-12 p-0">
            <div class="card card-outline-info">
                <div class="card-header">
                        <h4 class="m-b-0 text-white"><?php echo __d('admin', 'Approved Users'); ?></h4>
                 </div>
...
<?php echo "<div id='userdata'>";
  echo  '<div class="table-responsive m-t-0">';
         echo '<table id="approveduserstable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">';
                        echo '<thead>';
                        ?>
      ...
      <?php if(count($userdet)!=0){
            foreach($userdet as $user_det){
                ...
                echo '<tr id="del_'.$id.'" style="'.$style.'">';
                    ...
                    if($demo_active!='yes')
                    echo '<a onclick="deleteusrlists('.$id.',\'approveduser\')"><input type="button" class="btn btn-rounded btn-outline-danger" style="width:auto; font-size: 11px;" value="'.__d('admin', 'Delete').'"></a>';
                    echo ' </td>';
                echo '</tr>';
            }
            }else{
