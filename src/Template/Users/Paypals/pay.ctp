...
$_SESSION['redirectcart']=SITE_URL.'cart';
?>
<style type="text/css">
.cursoronhand
{
...
<?php
	...
	<div class="container">
		<section class="col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding">
				<h2 class="activity_heading txt-uppercase">
				<?php echo __d('user','You have').' '.$total_itms.' '.__d('user','items in your cart');?></h2>
		</section>
		<!--SHOP CART -->
		<section class="col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding margin-top10">
		<?php
		if(isset($_SESSION['payment_failure'])){ ?>
		<!--PAYMENT CANCEL NOTIFICATIONS-->
		<div class="clearfix payment-failure">
		<div class="trck-detail col-xs-12 col-sm-12 col-md-12 col-lg-12 padding-top10 padding-bottom10 hor-padding border">
		<div class="inline-block"><?php echo __d("user","Payment didn't go through Please try again");?> </div>
		</div>
		</div>
		<!--PAYMENT CANCEL NOTIFICATIONS-->
						  ...
						  <div class="hide_mobile_cart row hor-padding">
							...
							foreach($opencart as $setcart)
							{
								...
								/* MULTIPLE QUANTITTIES */
							 	$item_total = round($setcart['price'] * $setcart['qty'],2);
							 	$totalprice=0;
							 	$shipping=0;
							 	/* CART ITEM DETAILS */
								...
								<div class="cart_dis_table">
									...
									<div class="cart_dis_cell">
										...
										<div class="price margin-left20 margin-right20 margin-bottom5">
											<span class="hide_mobile_cart"><?php echo __d('user','Price').':';?></span>
										...
										<h6 class="text-left font_size13 margin-top5 margin-bottom5 margin-left20 margin-right20 text_right_rtl"><?php echo __d('user','Size').':'; ?> <span dir="rtl" class=""><?php if($setcart['size']!=""){ echo $setcart['size'];} else {echo 'N/A';} ?></span></h6>
										<div class=" margin-left20 text-left text_right_rtl"><?php echo __d('user','Seller').':';?>
										<a target="_blank" style="text-transform: capitalize;" href="<?php echo strtolower($baseurl.'stores/'.$setcart['shop_name']);?>" class="primary-color-txt cart_seller_border padding-right10 cursoronhand">
										<?php echo $setcart['sellername'];?></a>
										<a class="red-txt hide_mobile_cart cursoronhand padding-left10" onclick="removecart('<?php echo $setcart['cartid']; ?>','<?php echo $userid;?>','<?php echo $setcart['shopid'];?>','<?php echo $setcart['sellerid']; ?>')"><?php echo __d('user','Remove');?></a></div>
							...
							<div class="no_border col-xs-5 col-sm-2 col-md-2 col-lg-2 margin-top10 margin_top0_cart_mobile product_cnt">
								...
								<div class="no_border padding_left0_cart padding_right0_cart col-xs-7 col-sm-3 col-md-3 col-lg-3">
									...
									<?php
											if(isset($_SESSION['currency_code']))
											{	
												$item_total =$convertcurrency*$setcart['qty'];
												?>&#x200E;<?php echo $_SESSION['currency_symbol'].' '.$item_total;
											}
											else
											{
												if($itmscurrency>1){
							  ...
							  <div class="row hor-padding"><div class="text-left padding_table col-xs-12 col-md-12 col-sm-12 col-lg-12 text_right_rtl vertical_align_top margin-bottom20 padding-top10">
							  <span class="<?php echo 'errcls red-txt quantity_error'.$setcart['cartid'];?>"><?php echo __d('user','No more quantity available for this product');?></span>
							  <?php
							...
							<?php  $productId[] = $setcart['itemid']; } ?>
								...
								<div class="text-right col-xs-12 col-sm-4 col-md-4 col-lg-4 no-hor-padding" id="total_section">
										...
										class="subtotal">
										<?php 
											echo '<div style="direction:LTR">'.$item_price;?>
										<!--CONVERT CURRENCY STARTS-->
										<?php
										...
										<h6 class="font_size13"><?php echo __d('user','Shipping Total'); echo ':';?>&nbsp;<span class="subtotal"><?php echo '<div style="direction:LTR">'. $totalShippingPayable;?>
										<!--CONVERT CURRENCY STARTS-->
										<?php
												if(isset($_SESSION['currency_code']))
												{
										...
										<span class="subtotal"><?php echo '<div style="direction:LTR">'.round($indv_perc,2);?></span>
										<!--CONVERT CURRENCY STARTS-->
										<?php
												if(isset($_SESSION['currency_code']))
												{
										...
										<?php
										$totalAmountPayable=round(($item_price+$totalShippingPayable+$total_tax_value),2);?>
										<hr class="hr_total">
										<h6 class="font_size13 bold-font clear"><?php echo __d('user','Total'); echo':';?> &nbsp;<span class="grandtotal">
            ...
            <div class="resp-tabs-container hor_1">
                <div class="active_horizontal">
                   <div class="payment_dis_table margin-bottom15">
						...
						<?php echo $setngs['site_name'];?><?php echo __d('user',"'s");?> <a href="<?php echo SITE_URL.'help/terms_sales';?>" class="primary-color-txt"><?php echo __d('user','Terms of Use');?></a></span>
						</div>
				   </div>
				   <?php ?>
			...
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4 mobile_first padding_ship_address">
				<div class="ship_cart_address border_bottom_grey">
					<h4 class="bold-font margin-top0"><?php echo __d('user','Ship to');?></h4>
						...
						<?php
							if($default_count==0) { ?>
								<div class="centered-text font_size13 margin-top30 margin_bottom_mobile20">
					...
					<?php if($allproductship==0 && $soldout=='0') { $couponstyle="display:block"; } else { $couponstyle="display:none"; } ?>
					  <div class="panel panel-default applycouponcode" style="<?php echo $couponstyle;?>">
						...
						<a role="button" data-toggle="collapse" data-parent="#accordion_cart" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
						  <h4 class="panel-title accordion_cart bold-font no_hor_padding"><?php echo __d('user','Coupon Code')
							 ?><i class="more_less_cart fa fa-angle-up pull-right fa_font_wt"></i>
							</h4>
						  </a>
						</div>
					  ...
					  <div class="panel panel-default applygiftcard" style="<?php echo $giftcardstyle;?>">
						...
						<div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">
						   <div class="panel-body padding-top0">
							<input class="form-control font_size13" id="giftcode" 
							placeholder="<?php echo __d('user','Enter your Giftcard Code');?>">
