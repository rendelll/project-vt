	...
	if(($ext != '.JPG' && $ext != '.PNG' && $ext != '.JPEG' && $ext != '.GIF' && $ext != '.jpg' && $ext != '.png' && $ext != '.jpeg' && $ext != '.gif') || $fsize > 200*1024*1024){
		...
		if(isset($ftmp)){
			...
			<script>
				...
				par.getElementById('show_url2').src = '<?php echo $img_src; ?>';
				par.getElementById('image_shop').value = '<?php echo $newname; ?>';	
				var imagename = par.getElementById('image_shop').value;
				if(imagename){
					parent.document.getElementById('removeimg2').style.display='block';
				}
			</script>
			<?php
		}
...
function upload(){
	...
	par.getElementById('show_url2').src = '<?php echo $site_url."img/indicator.gif";?>';
}
</script>
...
<form id="iform" name="iform" class="settings" action="" method="post" enctype="multipart/form-data">
	<div class="row file-holder1">
		<div class="file file-input-js-active">
			<input type="file" value="Browse..." class="file-input-area1" id="file" style="opacity: 0;width:30px;filter: alpha(opacity = 0);" name="image" onchange="upload()"/>
