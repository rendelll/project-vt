...
function actionItem_datesearch()
{
	...
	var baseurl = BaseURL+"merchant/updateproducts/"+startdate+"/"+enddate;
	window.location = baseurl;
}
...
$(document).ready(function() {
  ...
  $("#itemform").submit(function(){
	...
	if(check == true)
	{
		if(imagecon==0 || imgname==""){
		...
		else if(IsDataValidation(title, emptymsg, 'title', 'length', 3, 70)  == false) check = false;
			...
			IsTimerror('description','8000',"This field must not exceeds 2500 charactes");
		 	check = false;
		}
	...
	if(check == true) {
		if(IsDataValidation(price, emptymsg, 'price', 'price', '', '') == false) check = false;
