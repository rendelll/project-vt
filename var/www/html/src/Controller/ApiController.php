...
class ApiController extends AppController{
	...
	function home()
	{
		...
		echo '{"status":"true","banner": '.$banner_detail.',"category": '.$category_detail.',"daily_deals": {"valid_till": '.$tdy.', "items":'.$daily_deals.'},"popular_products":'.$popular_products.',"recent_products":'.$recent_products.',"featured_products":'.$featured_products.',"popular_stores":'.$popular_stores.'}';
					die;
	}
...
function login()
{
	...
	if((!empty($email)) && (!empty($password))){
		...
		if($admin_data[0]['user_level'] == 'god'){
			 echo '{"status":"false","message":"You cannot login as Admin"}'; die;
			 return;
		}
		if($admin_data[0]['user_level'] == 'moderator'){
			 echo '{"status":"false","message":"You cannot login as Moderator"}'; die;
			 return;
		}
		if($admin_data[0]['user_level'] == 'shop'){
			 echo '{"status":"false","message":"You cannot login with merchant account"}'; die;
			 return;
		}
		...
		if(!empty($userdata)){
			...
			if ($userdata['activation'] == 1) {
				...
				if (!empty($_POST['devicetype'])){
					...
					echo '{"status":"true","user_id":"'.$userId.'","user_name":"'.$userName.'","user_image":"'.$fullImageName.'","full_name":"'.$fullname.'","first_time_logged":"'.$first_time_logged.'"}';die;

				} else {
					echo '{"status":"false","message":"Your account has been disbled please contact our support"}'; die;
				}
			} else {
	...
	function signup () {
		...
		function unfollowUser(){
			...
			if($flwalrdy > 0){
				...
				$this->Shops->updateAll(array('follow_count' => $flwrscnt), array('user_id' => $userid));
				echo '{"status":"true","message":"Successfully Unfollowed"}'; die;
