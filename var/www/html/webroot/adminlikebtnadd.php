...
if($ext){
	if($ext != '.JPG' && $ext != '.PNG' && $ext != '.JPEG' && $ext != '.GIF' && $ext != '.jpg' && $ext != '.png' && $ext != '.jpeg' && $ext != '.gif'){
		 $errmsg = 'Please upload only jpg,png,gif,jpeg images for Like Button logo';	
	}
	elseif($fsize > 10*1024*1024)
		{
			$errmsg = 'Please upload image upto 10MB for Like Button logo';
		}
	else{
		if(isset($ftmp)){
			$errmsg = '';
...
parent.document.getElementById('likebtnlogoerr').style.display = 'block';
parent.document.getElementById('likebtnlogoerr').innerHTML = '<?php echo $errmsg; ?>';
setTimeout(function() {
			 parent.document.getElementById('likebtnlogoerr').innerHTML = '';
			}, 5000);
</script>
